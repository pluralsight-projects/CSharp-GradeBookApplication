...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            if (parts.Length != 3)
            {
                Console.WriteLine("Command not valid, Create requires a name, type of gradebook, and if the grade book is weighted (true/false).");
                return;
            }
            var name = parts[1];
            var type = Enum.Parse(typeof(GradeBookType), parts[2]).ToString();
